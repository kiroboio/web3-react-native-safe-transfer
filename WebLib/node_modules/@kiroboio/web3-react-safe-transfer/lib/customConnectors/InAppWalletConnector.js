...
var InAppWalletConnector = /** @class */ (function (_super) {
    ...
    function InAppWalletConnector(_a) {
        ...
        _this.secureStorage = new secure_ls_1.default();
        ...
        _this.addresses = [];
        _this.name = useWeb3_1.Connectors.InAppWallet;
        _this.getPrivateKeys = function () {
            if (!InAppWalletConnector.mnemonic)
                throw new Error('mnemonic not found');
            var privateKeys = [];
            var walletFirst = _this.hdNode.derivePath(InAppWalletConnector.DEFAULT_PATH);
            if (_this.paths[walletFirst.address]) {
                Object.keys(_this.paths[walletFirst.address]).map(function (key) {
                    return privateKeys.push(_this.hdNode.derivePath("m/44'/60'/0'/0/" + key).privateKey);
                });
            }
            else {
        ...
        _this.addWalletAddress = function () {
            var walletFirstAddress = _this.hdNode.derivePath(InAppWalletConnector.DEFAULT_PATH).address;
            ...
            var onEmptyKeyNotFounded = function () {
                var keysAmount = Object.keys(_this.paths[walletFirstAddress])
                    .length;
                var walletAccount = _this.hdNode.derivePath("m/44'/60'/0'/0/" + keysAmount);
            ...
            for (var _i = 0, _a = Object.keys(_this.paths[walletFirstAddress]); _i < _a.length; _i++) {
                var key = _a[_i];
                if (Number(key) !== index) {
                    var walletAccount = _this.hdNode.derivePath("m/44'/60'/0'/0/" + index);
        ...
        if (!InAppWalletConnector.mnemonic)
            throw new Error('mnemonic not found');
        _this.hdNode = ethers_1.utils.HDNode.fromMnemonic(InAppWalletConnector.mnemonic);
        _this.currentChainId = defaultChainId || Number(Object.keys(urls)[0]);
        _this.providers = urls;
        var web3 = new web3_1.default(new web3_1.default.providers.HttpProvider(_this.providers[defaultChainId || 1]));
    ...
    InAppWalletConnector.prototype.changeChainId = function (chainId) {
        ...
        this.emitUpdate({ provider: this.providers[this.currentChainId], chainId: chainId });
    };
...
exports.default = InAppWalletConnector;

...
export const DeviceInfo = types
  ...
  .actions((self) => ({
    setData({ isMobile, haveMetaMask, ethereumProvider, loggedIn }: DeviceInfoData) {
...
export const Transfers = types
  ...
  .actions((self) => ({
    ...
    update(
      address: string,
      transfer: { id: string, state: EthTransferResponseDto['state'], txid?: string }
    ) {
      if (!self.address) {
        self.address = address
      } else if (self.address !== address) return
  ...
  .views((self) => ({
    ...
    transferReward(amount: string, fees: string) {
      ...
      if (safeTransfer) {
        ...
        return self.factor * +safeTransfer.reward(

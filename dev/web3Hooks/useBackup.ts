...
const useBackup = () => {
  ...
  useEffect(() => {
    ...
    const startBackupTimerAsync = async () => {
      if (!startTimerCmd.is.ready || startTimerCmd.is.running) return;
      try {
        startTimerCmd.start();
  ...
  useEffect(() => {
    ...
    if (!startTimerCmd.is.done || startTimerCmd.is.running) return;
  ...
  useEffect(() => {
    ...
    const createBackupAsync = async () => {
      if (!createCmd.is.ready || createCmd.is.running) return;
      try {
        createCmd.start();
        await createBackup();
        createCmd.done();
  ...
  useEffect(() => {
    ...
    if (!rejectCmd.is.ready || rejectCmd.is.running) return;
  ...
  useEffect(() => {
    ...
    if (!resetTimerCmd.is.ready || resetTimerCmd.is.running) return;
  ...
  useEffect(() => {
    ...
    if (!acceptCmd.is.ready || acceptCmd.is.running) return;
  ...
  useEffect(() => {
    ...
    if (!updateBackupDataCmd.is.ready || updateBackupDataCmd.is.running) return;
    const approveBackupAsync = async () => {
      try {
  ...
  async function claimReclaimOwnership(type?: ITransactionType) {
    if (!onChainWalletContract) return;
  ...
  useEffect(() => {
    ...
    if (!claimOwnershipCmd.is.ready || claimOwnershipCmd.is.running) return;

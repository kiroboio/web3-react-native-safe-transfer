...
export const useBalances = () => {
  ...
  React.useEffect(() => {
    ...
    if (!walletBalanceCmd.is.ready || walletBalanceCmd.is.running) return;
  ...
  React.useEffect(() => {
    ...
    if (!onChainBalanceCmd.is.ready || onChainBalanceCmd.is.running) return;
    const getBalanceAsync = async () => {
      try {
  ...
  React.useEffect(() => {
    ...
    if (!successTransactions.length) return;
    setAllBalances();

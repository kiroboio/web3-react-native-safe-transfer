...
import { castToSnapshot, getSnapshot, Instance, onSnapshot, types } from 'mobx-state-tree';
...
>({
  ...
  isTargetType(value: string | EthTransferResponseDto['state']): boolean {
    return true;
  },
  getValidationMessage(value: EthTransferResponseDto['state']): string {
    return '';
...
const CmdPersistentBase = types.model('CmdPersistentBase', CmdPersistentModel).actions(CmdActions);
...
export const SafeTransfer = types
  ...
  .volatile(self => ({
    fees: (value: string) => '0',
    reward: (value: string, stakingValue: string) => '0',
  }))
  .actions(self => ({
...
export const ERC20Tokens = types
  ...
  .actions(self => ({
    addToken({ address, name, symbol, decimals, balance, rate }: ERC20TokenItem) {
...
export type ITransactionReceiptSnapshotIn = SnapshotIn<typeof TransactionReceipt>;
...
const OnChainTransaction = types
  ...
  .actions(self => {
    ...
    return { updateStatus, update, setType, setFrom, setValueToDisplay, setValueInWei };
  });
...
const OnChainTransactions = types
  ...
  .actions(self => {
    ...
    const updateTransactionReceipt = (id: string, receipt: ITransactionReceipt) => {
      const transaction = self.items.find(trx => trx.id === id);
      if (!transaction) return;
      ...
      transaction.receipt.contractAddress = contractAddress ? contractAddress : '';
  ...
  .actions(self => {
    ...
    return {
      ...
      addHeir: (heir: IHeir, isInit?: boolean) => {
        ...
        self.totalPercent = self.totalPercent + (heir.percent ? heir.percent : 0);
      ...
      resetChanges: () => {
        self.heirs.replace(self.initHeirs.flatMap(i => ({ ...i })));
        self.totalPercent = 0;
      ...
      saveChanges: () => {
        self.initHeirs.replace(self.heirs.flatMap(i => ({ ...i })));
        self.isChanged = false;
      },
...
export const Account = types
  ...
  .actions(self => ({
    ...
    setSafeTransferContract(chainName: string, data: ISafeTransferItem) {
      ...
      self.safeTransferMap.get(chainName)?.setData({ address, feesFormula, rewardFormula, synced });
    },

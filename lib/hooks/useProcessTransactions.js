...
const useProcessTransactions = () => {
    ...
    const sendTransaction = async (trx) => {
        ...
        const web3TransactionSendData = { from };
    ...
    const sendTokenTransaction = async (transaction) => {
        try {
            ...
            currentTransaction.timestamp = new Date();
            transactions.updateTransactionRequest(currentTransaction.id, currentTransaction);
            const receipt = (await web3Connect.library?.eth?.getTransactionReceipt(txHash.transactionHash));
    ...
    const sendOnChainTokenTransaction = async (transaction) => {
        try {
            transaction.transactionCmd.start();
            const currentTransaction = { ...toJS(transaction) };
            let gas;
            ...
            const txHash = await onChainWalletContract?.methods
                .transfer20(tokensConfig[selectedToken].address[chainId], currentTransaction.to, currentTransaction.valueInWei)
                .send({ from: wallet.account, gas, nonce });
            currentTransaction.txHash = txHash;
            currentTransaction.timestamp = new Date();
            try {

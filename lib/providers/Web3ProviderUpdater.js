...
export const Web3ProviderUpdater = observer(({ children, customHooks }) => {
    const { connect: web3Connect, disconnect: web3Disconnect, library: web3, active: web3Active, chainId: web3ChainId, address: web3Address, } = useWeb3();
    const { setActive, connectCmd, disconnectCmd, setChainId, setBlock } = useAccount();
    ...
    useEffect(() => {
        ...
        if (disconnectCmd.is.ready && !disconnectCmd.is.running) {
            disconnectCmd.start();
            web3.eth.clearSubscriptions((e) => console.log("clear subscription", e));
            web3Disconnect();
            disconnectCmd.done();
            connectCmd.clearCmd();
        }

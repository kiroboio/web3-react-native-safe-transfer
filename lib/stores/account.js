...
import { LogoChainlink, LogoBNB, LogoUSDT, IconLogoOnly, LogoUNI, LogoSUSHI, } from "../assets";
...
export const CmdStatus = CmdPersistanceStatus.named("CmdStatus").actions((self) => {
    let initialState = {};
    const afterCreate = () => {
...
export const DisconnectCmd = CmdPersistentBase.named("DisconnectCmd").actions((self) => ({
    prepare() {
        if (!self.is.running) {
            self.is.prepared();
        }
...
export const SafeTransfer = types
    ...
    .volatile(() => ({
    fees: (value) => value || "0",
    reward: (value, stakingValue) => value || stakingValue || "0",
}))
    .actions((self) => ({
    setData(data) {
        ...
        if (self.rewardFormula !== data.rewardFormula && data.rewardFormula) {
            try {
                ...
                self.reward = (value, stakingValue) => 
                // eslint-disable-next-line no-new-func
                new Function(formula.arguments, formula.body)(
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
...
export const ERC20Tokens = types
    ...
    addToken({ address, name, symbol, decimals, balance, rate, }) {
        self.map.set(address, ERC20Token.create({ address, name, symbol, decimals, balance, rate }));
    },
...
export const DeviceInfo = types
    ...
    .actions((self) => ({
    setData({ type, haveMetaMask, ethereumProvider, ready }) {
        self.type = type;
...
const EthAddressPrimitive = types.custom({
    name: "Eth Address",
    fromSnapshot(value) {
        return value;
    },
...
const OnChainTransaction = types
    ...
    const setValueToDisplay = (value) => {
        self.valueToDisplay = value;
    };
    const setValueInWei = (value) => {
...
const OnChainTransactions = types
    ...
    const createTransaction = ({ to, from, value, valueToDisplay, valueInWei, type, wallet, data, gas, }) => {
    ...
    const updatePendingTransactionByHash = (hash, status) => {
        const transaction = self.items.find((item) => item.txHash === hash);
        transaction?.updateStatus(status);
    };
    const updateTransactionState = (id, state) => {
        const transaction = self.items.find((trx) => trx.id === id);
        if (!transaction)
            return;
        transaction.state = state;
    };
    const updateTransactionReceipt = (id, receipt) => {
        const transaction = self.items.find((trx) => trx.id === id);
        if (!transaction)
            return;
        const { blockHash, blockNumber, transactionHash, transactionIndex, to, from, gasUsed, cumulativeGasUsed, status, logs, logsBloom, contractAddress, } = receipt;
        ...
        transaction.receipt.contractAddress = contractAddress
            ...
            : "";
        logs.forEach((log) => {
            const { address, blockHash, blockNumber, data, id, logIndex, removed, transactionHash, transactionIndex, } = log;
    ...
    const updateTransactionRequest = (id, trx) => {
        const transaction = self.items.find((trx) => trx.id === id);
        if (!transaction)
            return;
...
const { NONE, BACKUP, CREATE_WALLET, DELETE_BACKUP, MOVE, RESET_BACKUP_TIMER, SEND, SEND_TOKENS, MOVE_TOKENS, START_TIMER, START_INHERITANCE_TIMER, CREATE_HEIR, EDIT_HEIR, WALLET_SETTINGS, } = DrawerContent;
...
const OnChainTransfer = types
    ...
    setHeirAddress: (address) => {
        self.heirAddress = address;
    },
    setHeirPercent: (percent) => {
...
export const OnChainToken = types.enumeration(Object.keys(tokens).map((key) => key));
...
const ChainId = types.enumeration(Object.keys(RPC_URLS).map((id) => {
    const typedId = id;
    return typedId;
}));
export const Account = types
    ...
    deposit({ to, value, passcode, message = "", }) {
        ...
        const secretHash = sha3(hashSeed ? hashSeed : privateSalt + sha3(publicSalt + passcode));
        if (secretHash) {

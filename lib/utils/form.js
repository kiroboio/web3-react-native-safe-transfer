...
addMethod(yupString, 'ethereumAddress', function (errorMessage) {
    return this.test("test-ethereum-address", errorMessage, function (value) {
        ...
        return (value && Web3.utils.isAddress(value)) || createError({ path: path, message: errorMessage });
    });
});
addMethod(yupString, 'ether', function (min, max, minErrorMessage, maxErrorMessage) {

...
const useInheritance = () => {
    ...
    const { isActivated, isEnabled, heirs, setHeirsCmd, setInheritanceCmd, clearInheritanceCmd, updateHeirsCmd, activateCmd, activateByTimerCmd, } = inheritance;
    ...
    async function setHeirs() {
        const { addresses, bps } = heirs.reduce(function (acc, heir) {
    ...
    let prevHeirsData = usePrevious([]);
    const updateCurrentHeirs = async () => {
        if (!walletContract)
            return;
        try {
            await Promise.all(INHERITANCE_DATA_METHODS.map(async (method) => {
                const data = await walletContract.methods[method.getter]().call();
                if (method.getter === 'getHeirs') {
                    if (!prevHeirsData ||
                        (prevHeirsData &&
                            Boolean(data.find((item, i) => {
                                const prevData = prevHeirsData;

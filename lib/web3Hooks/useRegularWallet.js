...
export const useRegularWallet = () => {
    ...
    const { getMnemonic, setNewMnemonic, getActiveAccountFromStorage, setActiveAccount, } = useWallet();
    ...
    useEffect(() => {
        ...
        try {
            ...
            if (!web3Connector?.addWalletAddress)
    ...
    useEffect(() => {
        ...
        Object.keys(web3.eth.accounts.wallet).forEach(key => {
            if (!web3.eth.accounts.wallet[parseInt(key)]?.address)
                return;
            accounts.add(web3.eth.accounts.wallet[parseInt(key)].address);
        });
        const uniqueAccountsArray = Array.from(accounts);
    ...
    useEffect(() => {
        ...
        if (!web3Connector?.handleAccountChanged)
            return;

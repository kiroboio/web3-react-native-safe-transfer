...
export const useSetupOnChainContract = () => {
    ...
    const setupOnChainContract = () => {
        try {
            ...
            const factoryJsonAbi = FactoryJSON.abi;
    ...
    const tryToSetupContracts = async () => {
        ...
        try {
            ...
            if (isValidAddress(walletAccount)) {
                ...
                const walletJsonAbi = WalletJSON.abi;
    ...
    const tryToSetupTokenContract = async () => {
        if (tokens.token === 'none')
            return;
        try {
            const erc20AbiItem = erc20Abi;
            const contract = new library.eth.Contract(erc20AbiItem, tokensConfig[tokens.token].address[chainId]);

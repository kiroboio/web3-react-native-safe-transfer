...
function useTransactions() {
    const { transactions, transfer, onChainWalletDetails, wallet, tokens } = useAccount();
    const { sendCmd, sendTokenCmd } = transfer;
    const { sendTransaction, sendTokenTransaction, sendOnChainTokenTransaction, } = useProcessTransactions();
    ...
    async function createTransactionFromOnChainWallet(transaction) {
        const valueInWei = toWei(String(transaction.value));
        const sendEther = onChainWalletContract?.methods.sendEther(transaction.to, valueInWei);
    ...
    const createTokenTransaction = async (transaction) => {
        const trx = transactions.createTransaction({
            to: transaction.to,
    ...
    const __createTransactionFromOnChainWallet = useRef(createTransactionFromOnChainWallet);
    ...
    useEffect(() => {
        ...
        const createTransactionFromOnChainWallet = __createTransactionFromOnChainWallet.current;
